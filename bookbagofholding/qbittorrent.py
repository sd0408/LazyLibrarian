...
from bookbagofholding.formatter import check_int, getList, makeBytestr, makeUnicode
...
class qbittorrentclient(object):
    ...
    def __init__(self):
        ...
        host = bookbagofholding.CONFIG['QBITTORRENT_HOST']
        port = check_int(bookbagofholding.CONFIG['QBITTORRENT_PORT'], 0)
        ...
        host = "%s:%s" % (host, port)
        self.base_url = host
        self.username = bookbagofholding.CONFIG['QBITTORRENT_USER']
        ...
        for torrent in torrentList:
            if torrent['hash'].lower() == hashid:
                ...
                if torrent['state'] == 'uploading' or torrent['state'] == 'stalledUP':
                    if not bookbagofholding.CONFIG['SEED_WAIT']:
...
def addTorrent(link, hashid):
    ...
    if not len(qbclient.cookiejar):
        ...
        logger.debug(res)
        return False, res
    dl_dir = bookbagofholding.CONFIG['QBITTORRENT_DIR']
    if dl_dir:
        args['savepath'] = dl_dir

    if bookbagofholding.CONFIG['QBITTORRENT_LABEL']:
        if 6 < qbclient.api < 10:
            args['label'] = bookbagofholding.CONFIG['QBITTORRENT_LABEL']
        elif qbclient.api >= 10:
            args['category'] = bookbagofholding.CONFIG['QBITTORRENT_LABEL']
