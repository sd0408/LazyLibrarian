# This file is modified to work with bookbagofholding by CurlyMo <curlymoo1@gmail.com>
...
def sendNZB(nzb=None, cmd=None, nzbID=None):
    ...
    host = bookbagofholding.CONFIG['NZBGET_HOST']
    port = check_int(bookbagofholding.CONFIG['NZBGET_PORT'], 0)
    ...
    url = hostparts[0] + '://' + nzbgetXMLrpc % {"host": hostparts[1], "username": bookbagofholding.CONFIG['NZBGET_USER'],
                                                 "port": port,
                                                 "password": bookbagofholding.CONFIG['NZBGET_PASS']}
    try:
    ...
    if cmd == "test":
        msg = "bookbagofholding connection test"
    elif nzbID:
        msg = "bookbagofholding connected to %s %s" % (cmd, nzbID)
    else:
        msg = "bookbagofholding connected to drop off %s any moment now." % (nzb.name + ".nzb")
    ...
    try:
        ...
        if nzbget_version == 0:  # or nzbget_version == 14:
            if nzbcontent64:
                nzbget_result = nzbGetRPC.append(nzb.name + ".nzb",
                                                 bookbagofholding.CONFIG['NZBGET_CATEGORY'], addToTop, nzbcontent64)
            else:
                # from bookbagofholding.common.providers.generic import GenericProvider
                ...
                #         return False
                #     nzbcontent64 = standard_b64encode(data)
                # nzbget_result = nzbGetRPC.append(nzb.name + ".nzb", bookbagofholding.CONFIG['NZBGET_CATEGORY'],
                #       addToTop, nzbcontent64)
                return False, "No nzbcontent64 found"
        elif nzbget_version == 12:
            if nzbcontent64:
                nzbget_result = nzbGetRPC.append(nzb.name + ".nzb", bookbagofholding.CONFIG['NZBGET_CATEGORY'],
                                                 bookbagofholding.CONFIG['NZBGET_PRIORITY'], False,
                                                 nzbcontent64, False, dupekey, dupescore, "score")
            else:
                nzbget_result = nzbGetRPC.appendurl(nzb.name + ".nzb", bookbagofholding.CONFIG['NZBGET_CATEGORY'],
                                                    bookbagofholding.CONFIG['NZBGET_PRIORITY'], False, nzb.url, False,
        ...
        # (Positive number representing NZBID of the queue item. 0 and negative numbers represent error codes.)
            ...
            nzbget_result = nzbGetRPC.append(nzb.name + ".nzb", nzbcontent64 if nzbcontent64 is not None else nzb.url,
                                             bookbagofholding.CONFIG['NZBGET_CATEGORY'],
                                             bookbagofholding.CONFIG['NZBGET_PRIORITY'], False, False, dupekey, dupescore,
                                             "score")
            if nzbget_result <= 0:
                nzbget_result = False
        else:
            if nzbcontent64:
                nzbget_result = nzbGetRPC.append(nzb.name + ".nzb", bookbagofholding.CONFIG['NZBGET_CATEGORY'],
                                                 bookbagofholding.CONFIG['NZBGET_PRIORITY'], False, nzbcontent64)
            else:
                nzbget_result = nzbGetRPC.appendurl(nzb.name + ".nzb", bookbagofholding.CONFIG['NZBGET_CATEGORY'],
                                                    bookbagofholding.CONFIG['NZBGET_PRIORITY'], False, nzb.url)
