...
from bookbagofholding.bookrename import audioProcess, nameVars
from bookbagofholding.bookwork import setWorkPages, getWorkPage, getBookAuthors, setAllBookAuthors, \
    setWorkID, get_book_desc
from bookbagofholding.cache import cache_img
from bookbagofholding.calibre import syncCalibreList, calibreList
from bookbagofholding.common import clearLog, cleanCache, restartJobs, showJobs, checkRunningJobs, aaUpdate, setperm, \
    logHeader, authorUpdate, showStats
from bookbagofholding.csvfile import import_CSV, export_CSV, dump_table
from bookbagofholding.formatter import formatAuthorName, check_int, plural, makeUnicode, makeBytestr
from bookbagofholding.gb import GoogleBooks
from bookbagofholding.images import getAuthorImage, getAuthorImages, getBookCover, getBookCovers
...
from bookbagofholding.postprocess import processDir, processAlternate, processOPF, processIMG, getDownloadProgress, delete_task
from bookbagofholding.providers import get_capabilities
from bookbagofholding.rssfeed import genFeed
from bookbagofholding.searchbook import search_book
from bookbagofholding.searchrss import search_rss_book, search_wishlist
...
cmd_dict = {'help': 'list available commands. ' +
            ...
            'checkModules': 'Check using bookbagofholding library modules',
            ...
            'getVersion': 'show bookbagofholding version',
            ...
            'shutdown': 'stop bookbagofholding',
            'restart': 'restart bookbagofholding',
            ...
            'cleanCache': '[&wait] Clean unused and expired files from the Bookbag of Holding caches',
            ...
            'logMessage': '&level= &text=  send a message to bookbagofholding logger',
            }
...
class Api(object):
    ...
    def checkParams(self, **kwargs):
        ...
        if not bookbagofholding.CONFIG['API_ENABLED']:
        ...
        provided_key = kwargs['apikey']
        global_key = bookbagofholding.CONFIG.get('API_KEY', '')

        # Check global API key first (admin access)
        if global_key and len(global_key) == 32:
            if provided_key == global_key:
                self.apikey = kwargs.pop('apikey')
                self.user_perms = bookbagofholding.perm_admin
                self.api_user = 'admin'
            else:
                # Global key is configured but provided key doesn't match
                # Try per-user API key
                try:
                    myDB = database.DBConnection()
                    user = myDB.match(
                        'SELECT UserID, UserName, Perms FROM users WHERE ApiKey=?',
                        (provided_key,)
                    )
                    if user:
                        self.apikey = kwargs.pop('apikey')
                        self.user_perms = user['Perms'] or 0
                        self.api_user = user['UserName']
                    else:
            ...
            # No global key configured - check for per-user API keys only
            try:
                myDB = database.DBConnection()
                user = myDB.match(
                    'SELECT UserID, UserName, Perms FROM users WHERE ApiKey=?',
            ...
            # Global key configured but invalid length
    ...
    def _showMonths(self):
        self.data = bookbagofholding.MONTHNAMES
    ...
    def _saveTable(self, **kwargs):
        ...
        if kwargs['table'] not in valid:
            ...
            return
        self.data = "Saved %s" % dump_table(kwargs['table'], bookbagofholding.DATADIR)
    ...
    @staticmethod
    def _dumpMonths():
        json_file = os.path.join(bookbagofholding.DATADIR, 'monthnames.json')
        with open(json_file, 'w') as f:
            json.dump(bookbagofholding.MONTHNAMES, f)
    ...
    def _getLogs(self):
        self.data = bookbagofholding.LOGLIST
    ...
    def _checkModules(self):
        ...
        for item in sys.modules:
            ...
            for libname in ['apscheduler', 'bs4', 'cherrypy_cors', 'feedparser', 'fuzzywuzzy', 'html5lib',
                            'httplib2', 'magic', 'mobi', 'requests', 'rfeed', 'tinytag',
                            'unrar', 'webencodings']:
    ...
    def _forceBookSearch(self, **kwargs):
        if 'type' in kwargs:
            ...
            library = None
        if bookbagofholding.USE_NZB() or bookbagofholding.USE_TOR() or bookbagofholding.USE_RSS() or bookbagofholding.USE_DIRECT():
            if 'wait' in kwargs:
    ...
    def _getVersion(self):
        from bookbagofholding.version import BOOKBAGOFHOLDING_VERSION
        self.data = {
            'Success': True,
            'install_type': BOOKBAGOFHOLDING_VERSION,
            'current_version': BOOKBAGOFHOLDING_VERSION,
            'latest_version': BOOKBAGOFHOLDING_VERSION,
            'commits_behind': 0,
        }
    ...
    def _addProvider(self, **kwargs):
        ...
        if prov_type == 'newznab':
            prov_list = bookbagofholding.NEWZNAB_PROV
            prov_prefix = 'Newznab'
            default_bookcat = '7000,7020'
            default_audiocat = '3030'
        else:
            prov_list = bookbagofholding.TORZNAB_PROV
            prov_prefix = 'Torznab'
        ...
        if slot_index is None:
            ...
            if prov_type == 'newznab':
                bookbagofholding.add_newz_slot()
            else:
                bookbagofholding.add_torz_slot()
        ...
        bookbagofholding.config_write(prov_name)
    ...
    def _delProvider(self, **kwargs):
        ...
        found = False
        for prov_list in [bookbagofholding.NEWZNAB_PROV, bookbagofholding.TORZNAB_PROV]:
            for prov in prov_list:
                if prov.get('NAME', '').lower() == name.lower() or prov.get('DISPNAME', '').lower() == name.lower():
                    prov['HOST'] = ''
                    prov['API'] = ''
                    prov['ENABLED'] = 0
                    bookbagofholding.config_write(prov['NAME'])
                    found = True
                    break
            if found:
                break
    ...
    def _changeProvider(self, **kwargs):
        ...
        bookbagofholding.config_write(found_prov['NAME'])
    ...
    def _writeCFG(self, **kwargs):
        ...
        if 'group' not in kwargs:
            ...
            return
        try:
            self.data = '["%s"]' % bookbagofholding.CFG.get(kwargs['group'], kwargs['name'])
            bookbagofholding.CFG.set(kwargs['group'], kwargs['name'], kwargs['value'])
            with open(bookbagofholding.CONFIGFILE, 'w') as configfile:
                bookbagofholding.CFG.write(configfile)
            bookbagofholding.config_read(reloaded=True)
        except Exception as e:
    ...
    def _setimage(self, table, itemid, img):
        ...
        if os.path.isfile(img):
            ...
            if extn and extn in ['.jpg', '.jpeg', '.png']:
                destfile = os.path.join(bookbagofholding.CACHEDIR, table, itemid + '.jpg')
                try:
    ...
    def _importAlternate(self, **kwargs):
        if 'dir' in kwargs:
            ...
            usedir = bookbagofholding.CONFIG['ALTERNATE_DIR']
        if 'wait' in kwargs:
    ...
    def _includeAlternate(self, **kwargs):
        if 'dir' in kwargs:
            ...
            startdir = bookbagofholding.CONFIG['ALTERNATE_DIR']
        if 'wait' in kwargs:
