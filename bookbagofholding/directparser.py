...
from bookbagofholding.formatter import plural, formatAuthorName, makeUnicode, size_in_bytes, url_fix
...
def GEN(book=None, prov=None, test=False):
    ...
    if not prov:
        prov = 'GEN'
    host = bookbagofholding.CONFIG[prov + '_HOST']
    if not host.startswith('http'):
        host = 'http://' + host

    search = bookbagofholding.CONFIG[prov + '_SEARCH']
                    ...
                    if link and title:
                        ...
                        if url:
                            results.append({
                                ...
                                'tor_size': str(size),
                                'tor_type': 'direct',
                                'priority': bookbagofholding.CONFIG[prov + '_DLPRIORITY']
                            })
