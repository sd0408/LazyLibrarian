...
from bookbagofholding.common import getUserAgent
from bookbagofholding.formatter import check_int, getList
...
class utorrentclient(object):
    ...
    def __init__(self, base_url='',  # bookbagofholding.CONFIG['UTORRENT_HOST'],
                 username='',  # bookbagofholding.CONFIG['UTORRENT_USER'],
                 password='',):  # bookbagofholding.CONFIG['UTORRENT_PASS']):

        host = bookbagofholding.CONFIG['UTORRENT_HOST']
        port = check_int(bookbagofholding.CONFIG['UTORRENT_PORT'], 0)
        ...
        host = "%s:%s" % (host, port)
        self.base_url = host
        self.username = bookbagofholding.CONFIG['UTORRENT_USER']
        self.password = bookbagofholding.CONFIG['UTORRENT_PASS']
    ...
    def _action(self, params, body=None, content_type=None):
        ...
        if bookbagofholding.CONFIG['PROXY_HOST']:
            for item in getList(bookbagofholding.CONFIG['PROXY_TYPE']):
                request.set_proxy(bookbagofholding.CONFIG['PROXY_HOST'], item)
...
def checkLink():
    ...
    try:
        ...
        if client.token:
            # we would also like to check bookbagofholding.utorrent_label
            ...
            if bookbagofholding.CONFIG['UTORRENT_LABEL']:
...
def labelTorrent(hashid):
    label = bookbagofholding.CONFIG['UTORRENT_LABEL']
