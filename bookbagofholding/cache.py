...
from bookbagofholding.common import getUserAgent, proxyList
from bookbagofholding.formatter import check_int, md5_utf8, makeBytestr, makeUnicode, seconds_to_midnight
...
def fetchURL(URL, headers=None, retry=True, raw=None):
    ...
    try:
        ...
        if '/torznab/' in URL and ('/all/' in URL or '/aggregate/' in URL):
            timeout = check_int(bookbagofholding.CONFIG['HTTP_EXT_TIMEOUT'], 90)
        else:
...
def cache_img(img_type, img_ID, img_url, refresh=False):
    ...
    cachefile = os.path.join(bookbagofholding.CACHEDIR, img_type, img_ID + '.jpg')
    link = 'cache/%s/%s.jpg' % (img_type, img_ID)
    if os.path.isfile(cachefile) and not refresh:  # overwrite any cached image
        if bookbagofholding.LOGLEVEL & bookbagofholding.log_cache:
