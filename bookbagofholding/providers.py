...
from bookbagofholding.directparser import GEN
from bookbagofholding.formatter import age, today, plural, cleanName, unaccented, getList, check_int, \
    makeUnicode, seconds_to_midnight
from bookbagofholding.torrentparser import KAT, TPB, WWT, ZOO, TDL, LIME
...
def test_provider(name, host=None, api=None):
    ...
    if name == 'TPB':
        ...
        if host:
            bookbagofholding.CONFIG['TPB_HOST'] = host
        return TPB(book, test=True), "Pirate Bay"
    if name == 'WWT':
        logger.debug("Testing provider %s" % name)
        if host:
            bookbagofholding.CONFIG['WWT_HOST'] = host
        return WWT(book, test=True), "WorldWideTorrents"
    if name == 'KAT':
        logger.debug("Testing provider %s" % name)
        if host:
            bookbagofholding.CONFIG['KAT_HOST'] = host
        return KAT(book, test=True), "KickAss Torrents"
    if name == 'ZOO':
        logger.debug("Testing provider %s" % name)
        if host:
            bookbagofholding.CONFIG['ZOO_HOST'] = host
        return ZOO(book, test=True), "Zooqle"
    if name == 'LIME':
        logger.debug("Testing provider %s" % name)
        if host:
            bookbagofholding.CONFIG['LIME_HOST'] = host
        return LIME(book, test=True), "LimeTorrents"
    if name == 'TDL':
        logger.debug("Testing provider %s" % name)
        if host:
            bookbagofholding.CONFIG['TDL_HOST'] = host
        return TDL(book, test=True), "TorrentDownloads"
    if name == 'GEN':
        logger.debug("Testing provider %s" % name)
        if host:
            bookbagofholding.CONFIG['GEN_HOST'] = host
        if api:
            bookbagofholding.CONFIG['GEN_SEARCH'] = api
        return GEN(book, prov='GEN', test=True), "LibGen 1"
    if name == 'GEN2':
        logger.debug("Testing provider %s" % name)
        if host:
            bookbagofholding.CONFIG['GEN2_HOST'] = host
        if api:
            bookbagofholding.CONFIG['GEN2_SEARCH'] = api
        return GEN(book, prov='GEN2', test=True), "LibGen 2"
    if name.startswith('rss_'):
        try:
            prov = name.split('_')[1]
            for provider in bookbagofholding.RSS_PROV:
...
def get_capabilities(provider, force=False):
    ...
    if not force and len(provider['UPDATED']) == 10:  # any stored values?
        match = True
        if (age(provider['UPDATED']) > bookbagofholding.CONFIG['CACHE_AGE']) and not provider['MANUAL']:
    ...
    if match:
        ...
        if not success:
            ...
            if provider['BOOKCAT'] or provider['MAGCAT'] or provider['AUDIOCAT']:
                ...
                provider['APILIMIT'] = 0
                bookbagofholding.config_write(provider['NAME'])
    ...
    for entry in bookbagofholding.PROVIDER_BLOCKLIST:
        if entry["name"] == name:
            if timenow < int(entry['resume']):
                ...
                bookbagofholding.PROVIDER_BLOCKLIST.remove(entry)
    return False


def BlockProvider(who, why, delay=0):
    if not delay:
        delay = check_int(bookbagofholding.CONFIG['BLOCKLIST_TIMER'], 3600)
    if len(why) > 40:
        why = why[:40] + '...'
    if delay == 0:
        ...
        bookbagofholding.PROVIDER_BLOCKLIST.append(newentry)
    logger.debug("Provider Blocklist contains %s entries" % len(bookbagofholding.PROVIDER_BLOCKLIST))
...
def IterateOverTorrentSites(book=None, searchType=None):
    ...
    for prov in ['KAT', 'TPB', 'WWT', 'ZOO', 'TDL', 'LIME']:
        if bookbagofholding.CONFIG[prov]:
            if ProviderIsBlocked(prov):
                logger.debug('[IterateOverTorrentSites] - %s is BLOCKED' % bookbagofholding.CONFIG[prov + '_HOST'])
            elif searchType in ['book', 'shortbook'] and 'E' not in bookbagofholding.CONFIG[prov + '_DLTYPES']:
                logger.debug("Ignoring %s for eBook" % prov)
            elif "audio" in searchType and 'A' not in bookbagofholding.CONFIG[prov + '_DLTYPES']:
                logger.debug("Ignoring %s for AudioBook" % prov)
            elif "mag" in searchType and 'M' not in bookbagofholding.CONFIG[prov + '_DLTYPES']:
                logger.debug("Ignoring %s for Magazine" % prov)
            else:
                logger.debug('[IterateOverTorrentSites] - %s' % bookbagofholding.CONFIG[prov + '_HOST'])
                if prov == 'KAT':
...
def IterateOverDirectSites(book=None, searchType=None):
    ...
    for prov in ['GEN', 'GEN2']:
        if bookbagofholding.CONFIG[prov]:
            if ProviderIsBlocked(prov):
                logger.debug('[IterateOverDirectSites] - %s %s is BLOCKED' % (bookbagofholding.CONFIG[prov + '_HOST'],
                                                                              bookbagofholding.CONFIG[prov + '_SEARCH']))
            elif searchType in ['book', 'shortbook'] and 'E' not in bookbagofholding.CONFIG[prov + '_DLTYPES']:
                logger.debug("Ignoring %s for eBook" % prov)
            elif "audio" in searchType and 'A' not in bookbagofholding.CONFIG[prov + '_DLTYPES']:
                logger.debug("Ignoring %s for AudioBook" % prov)
                ...
                logger.debug('[IterateOverDirectSites] - %s %s' % (bookbagofholding.CONFIG[prov + '_HOST'],
                                                                   bookbagofholding.CONFIG[prov + '_SEARCH']))
...
def IterateOverRSSSites():
    ...
    dltypes = ''
    for provider in bookbagofholding.RSS_PROV:
        if provider['ENABLED'] and not bookbagofholding.WishListType(provider['HOST']):
            if ProviderIsBlocked(provider['HOST']):
...
def IterateOverWishLists():
    ...
    for provider in bookbagofholding.RSS_PROV:
        if provider['ENABLED']:
            wishtype = bookbagofholding.WishListType(provider['HOST'])
...
def RSS(host=None, feednr=None, priority=0, dispname=None, types='E', test=False):
    ...
    if data:
        ...
        for post in data.entries:
            ...
            if magnet:
                if not url or (url and bookbagofholding.CONFIG['PREFER_MAGNET']):
                    url = magnet
                    tortype = 'magnet'
...
def cancelSearchType(searchType, errorMsg, provider):
    ...
    if (provider['BOOKSEARCH'] and searchType in ["book", "shortbook"]) or \
        ...
        if match:
            ...
            if msg:
                for providerlist in [bookbagofholding.NEWZNAB_PROV, bookbagofholding.TORZNAB_PROV]:
                    count = 0
    ...
    if params:
        ...
        if rootxml is not None:
            ...
            if rootxml.tag == 'error':
                ...
                nzbcount = 0
                maxage = check_int(bookbagofholding.CONFIG['USENET_RETENTION'], 0)
                minimumseeders = check_int(bookbagofholding.CONFIG['NUMBEROFSEEDERS'], 0)
...
def ReturnResultsFieldsBySearchType(book=None, nzbdetails=None, host=None, searchMode=None, priority=0):
    ...
    while n < len(nzbdetails):
        ...
        if tag == 'title':
            ...
            if not nzburl or (nzburl and not bookbagofholding.CONFIG['PREFER_MAGNET']):
