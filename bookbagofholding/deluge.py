...
# Adapted for Bookbag of Holding by Phil Borman
...
from bookbagofholding.common import mymakedirs
...
def _get_auth():
    ...
    delugeweb_cert = bookbagofholding.CONFIG['DELUGE_CERT']
    delugeweb_host = bookbagofholding.CONFIG['DELUGE_HOST']
    delugeweb_port = check_int(bookbagofholding.CONFIG['DELUGE_PORT'], 0)
    if not delugeweb_host or not delugeweb_port:
        logger.error('Invalid delugeweb host or port, check your config')
        return None

    delugeweb_password = bookbagofholding.CONFIG['DELUGE_PASS']
    ...
    if bookbagofholding.CONFIG['DELUGE_BASE']:
        delugeweb_base = bookbagofholding.CONFIG['DELUGE_BASE'].strip('/')
...
def setTorrentLabel(result):
    ...
    label = bookbagofholding.CONFIG['DELUGE_LABEL']
    if not any(delugeweb_auth):
        _get_auth()
...
def setTorrentPath(result):
    ...
    dl_dir = bookbagofholding.CONFIG['DELUGE_DIR']
