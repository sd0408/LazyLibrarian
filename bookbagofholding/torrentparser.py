...
from bookbagofholding.formatter import plural, unaccented, makeUnicode, size_in_bytes, url_fix
...
def TPB(book=None, test=False):
    errmsg = ''
    provider = "TPB"
    host = bookbagofholding.CONFIG['TPB_HOST']
    ...
    page = 0
    results = []
    minimumseeders = int(bookbagofholding.CONFIG['NUMBEROFSEEDERS']) - 1
    next_page = True
    ...
    while next_page:
                        ...
                        if minimumseeders < seeders:
                            ...
                            if not magnet or not title:
                                ...
                                results.append({
                                    ...
                                    'tor_size': str(size),
                                    'tor_type': 'magnet',
                                    'priority': bookbagofholding.CONFIG['TPB_DLPRIORITY']
                                })
        ...
        page += 1
        if 0 < bookbagofholding.CONFIG['MAX_PAGES'] < page:
...
def KAT(book=None, test=False):
    errmsg = ''
    provider = "KAT"
    host = bookbagofholding.CONFIG['KAT_HOST']
    if not host.startswith('http'):
        host = 'http://' + host
        ...
        for row in rows:
            ...
            if len(td) > 3:
                try:
                    ...
                    if not url or (magnet and url and bookbagofholding.CONFIG['PREFER_MAGNET']):
                        url = magnet
                        mode = 'magnet'
                    ...
                    if not url or not title:
                        ...
                        results.append({
                            ...
                            'tor_size': str(size),
                            'tor_type': mode,
                            'priority': bookbagofholding.CONFIG['KAT_DLPRIORITY']
                        })
...
def WWT(book=None, test=False):
    ...
    host = bookbagofholding.CONFIG['WWT_HOST']
    if not host.startswith('http'):
        host = 'http://' + host
                    ...
                    try:
                        ...
                        if not url or not title:
                            ...
                            results.append({
                                ...
                                'tor_size': str(size),
                                'tor_type': mode,
                                'priority': bookbagofholding.CONFIG['WWT_DLPRIORITY']
                            })
                            logger.debug('Found %s. Size: %s' % (title, size))
                            next_page = True
                        else:
...
def EXTRA(book=None, test=False):
    ...
    host = bookbagofholding.CONFIG['EXTRA_HOST']
    if not host.startswith('http'):
        host = 'http://' + host
                ...
                try:
                    ...
                    if not url or not title:
                        ...
                        results.append({
                            ...
                            'tor_size': str(size),
                            'tor_type': 'torrent',
                            'priority': bookbagofholding.CONFIG['EXTRA_DLPRIORITY']
                        })
                        logger.debug('Found %s. Size: %s' % (title, size))
                    else:
                        logger.debug('Found %s but %s seeder%s' % (title, seeders, plural(seeders)))
...
def ZOO(book=None, test=False):
    errmsg = ''
    provider = "zooqle"
    host = bookbagofholding.CONFIG['ZOO_HOST']
    if not host.startswith('http'):
        host = 'http://' + host
                ...
                try:
                    ...
                    if not url or not title:
                        ...
                        results.append({
                            ...
                            'tor_size': str(size),
                            'tor_type': mode,
                            'priority': bookbagofholding.CONFIG['ZOO_DLPRIORITY']
                        })
                        logger.debug('Found %s. Size: %s' % (title, size))
                    else:
                        logger.debug('Found %s but %s seeder%s' % (title, seeders, plural(seeders)))
...
def LIME(book=None, test=False):
    ...
    host = bookbagofholding.CONFIG['LIME_HOST']
    if not host.startswith('http'):
        host = 'http://' + host
                ...
                try:
                    ...
                    if not url or not title:
                        ...
                        res = {
                            ...
                            'tor_size': str(size),
                            'tor_type': 'torrent',
                            'priority': bookbagofholding.CONFIG['LIME_DLPRIORITY']
                        }
                        if pubdate:
...
def TDL(book=None, test=False):
    ...
    host = bookbagofholding.CONFIG['TDL_HOST']
    if not host.startswith('http'):
        host = 'http://' + host
                    ...
                    if link and minimumseeders < seeders:
                        ...
                        if not url or not title:
                            ...
                            res = {
                                ...
                                'tor_size': str(size),
                                'tor_type': 'magnet',
                                'priority': bookbagofholding.CONFIG['TDL_DLPRIORITY']
                            }
                            if pubdate:
                                res['tor_date'] = pubdate
                            logger.debug('Found %s. Size: %s' % (title, size))
