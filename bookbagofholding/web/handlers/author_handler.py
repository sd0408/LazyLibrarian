...
"""
Author-related web handlers for Bookbag of Holding.
...
from bookbagofholding.formatter import check_int, safe_unicode
...
from bookbagofholding.web.templates import serve_template
...
class AuthorHandler:
    ...
    def get_author_page(author_id: str, book_lang: Optional[str] = None,
        ...
        return serve_template(
            ...
            library=library,
            ignored=ignored,
            showseries=bookbagofholding.SHOW_SERIES,
            firstpage=firstpage
        )
    ...
    def toggle_ignored_authors() -> None:
        ...
        Raises:
            cherrypy.HTTPRedirect: To home page
        """
        bookbagofholding.IGNORED_AUTHORS = not bookbagofholding.IGNORED_AUTHORS
        raise cherrypy.HTTPRedirect("home")
        ...
        if authorsearch:
            ...
            if library == 'AudioBook':
                authordir = safe_unicode(
                    os.path.join(bookbagofholding.DIRECTORY('AudioBook'), author_name)
                )
            else:
                authordir = safe_unicode(
                    os.path.join(bookbagofholding.DIRECTORY('eBook'), author_name)
                )
            ...
            if not os.path.isdir(authordir):
                ...
                if library == 'AudioBook':
                    authordir = safe_unicode(
                        os.path.join(bookbagofholding.DIRECTORY('Audio'), author_name_cap)
                    )
                else:
                    authordir = safe_unicode(
                        os.path.join(bookbagofholding.DIRECTORY('eBook'), author_name_cap)
                    )
...
def _cache_author_image(author_id: str, image_path: str) -> Tuple[str, bool]:
    ...
    """
    if image_path == 'none':
        return os.path.join(bookbagofholding.PROG_DIR, 'data', 'images', 'nophoto.png'), True

    # Try local file
    if os.path.isfile(image_path):
        extn = os.path.splitext(image_path)[1].lower()
        if extn in ['.jpg', '.jpeg', '.png']:
            destfile = os.path.join(bookbagofholding.CACHEDIR, 'author', author_id + '.jpg')
            try:
