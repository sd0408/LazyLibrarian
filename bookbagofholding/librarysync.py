...
from bookbagofholding.database import add_unmatched_file, cleanup_unmatched_files, dedupe_unmatched_files
from bookbagofholding.bookwork import setWorkPages
from bookbagofholding.bookrename import bookRename, audioProcess, id3read
from bookbagofholding.cache import cache_img
from bookbagofholding.common import opf_file, any_file
from bookbagofholding.formatter import plural, is_valid_isbn, is_valid_booktype, getList, unaccented, \
    cleanName, replace_all, split_title, now, makeUnicode, makeBytestr
from bookbagofholding.gb import GoogleBooks
from bookbagofholding.importer import update_totals, addAuthorNameToDB
from fuzzywuzzy import fuzz
from bookbagofholding.mobi import Mobi
...
def find_book_in_db(author, book, ignored=None, library='eBook'):
    ...
    if match:
        ...
        if bookbagofholding.LOGLEVEL & bookbagofholding.log_libsync:
...
def LibraryScan(startdir=None, library='eBook', authid=None, remove=True):
    ...
    destdir = bookbagofholding.DIRECTORY(library)
    if not startdir:
        if not destdir:
    ...
    try:
        ...
        if startdir == destdir:
            bookbagofholding.AUTHORS_UPDATE = 1
            if library == 'eBook':
                bookbagofholding.EBOOK_UPDATE = 1
            elif library == 'AudioBook':
                bookbagofholding.AUDIO_UPDATE = 1
        ...
        if remove:
            if library == 'eBook':
                ...
                status = bookbagofholding.CONFIG['NOTFOUND_STATUS']
        ...
        matchString = ''
        for char in bookbagofholding.CONFIG['EBOOK_DEST_FILE']:
        ...
        for rootdir, dirnames, filenames in os.walk(makeBytestr(startdir)):
            ...
            for files in filenames:
                ...
                if library == 'eBook' and bookbagofholding.CONFIG['IMP_SINGLEBOOK'] and \
                    ...
                    if (library == 'eBook' and is_valid_booktype(files, 'ebook')) or \
                        ...
                        if match:
                            ...
                            if author:
                                ...
                                if not bookid:
                                    # GoogleBooks doesn't have the book. No point in looking for it again.
                                        ...
                                        if library == 'eBook':
                                            if check_status['Status'] != 'Open':
                                                ...
                                                myDB.action(
                                                    ...
                                                    (bookbagofholding.CONFIG['FOUND_STATUS'], now(), bookid))
                                ...
                                if not warned_no_new_authors and not bookbagofholding.CONFIG['ADD_AUTHOR']:
        ...
        if startdir == destdir:
            ...
            if bookbagofholding.CONFIG['BOOK_API'] == "GoogleBooks":
            ...
            logger.debug("Cache %s hit%s, %s miss" %
                         (bookbagofholding.CACHE_HIT, plural(bookbagofholding.CACHE_HIT), bookbagofholding.CACHE_MISS))
            ...
            if library == 'eBook':
                bookbagofholding.EBOOK_UPDATE = 0
            elif library == 'AudioBook':
                bookbagofholding.AUDIO_UPDATE = 0
            bookbagofholding.AUTHORS_UPDATE = 0
        else:
            if authid:
                match = myDB.match('SELECT authorid from authors where authorid=?', (authid,))
                if match:
