...
def getServer():
    host = bookbagofholding.CONFIG['RTORRENT_HOST']
    if not host:
    ...
    if bookbagofholding.CONFIG['RTORRENT_USER']:
        user = bookbagofholding.CONFIG['RTORRENT_USER']
        password = bookbagofholding.CONFIG['RTORRENT_PASS']
        parts = host.split('://')
...
def addTorrent(tor_url, hashID, data=None):
    ...
    try:
        ...
        label = bookbagofholding.CONFIG['RTORRENT_LABEL']
        if label:
            server.d.set_custom1(hashID, label)

        directory = bookbagofholding.CONFIG['RTORRENT_DIR']
        if directory:
