...
#  Adapted for Bookbag of Holding from Mylar
...
from bookbagofholding.formatter import makeUnicode, check_int, plural, getList
from bookbagofholding.common import mimeType, zipAudio
from bookbagofholding.cache import cache_img
from urllib.parse import quote_plus


searchable = ['Authors', 'Author', 'RecentBooks', 'RecentAudio',
              'RatedBooks', 'RatedAudio', 'ReadBooks', 'ToReadBooks', 'Genre', 'Genres']

cmd_list = searchable + ['root', 'Serve', 'search']
...
class OPDS(object):
    ...
    def __init__(self):
        ...
        self.PAGE_SIZE = check_int(bookbagofholding.CONFIG['OPDS_PAGE'], 0)

        if bookbagofholding.CONFIG['HTTP_ROOT'] is None:
            self.opdsroot = '/opds'
        elif bookbagofholding.CONFIG['HTTP_ROOT'].endswith('/'):
            self.opdsroot = bookbagofholding.CONFIG['HTTP_ROOT'] + 'opds'
        else:
            if bookbagofholding.CONFIG['HTTP_ROOT'] != '/':
                self.opdsroot = bookbagofholding.CONFIG['HTTP_ROOT'] + '/opds'
            else:
                self.opdsroot = bookbagofholding.CONFIG['HTTP_ROOT'] + 'opds'
    ...
    def checkParams(self, **kwargs):
        ...
        if not bookbagofholding.CONFIG['OPDS_ENABLED']:
    ...
    def fetchData(self):
        if self.data == 'OK':
            ...
            if isinstance(self.data, str):
                ...
                return bookbagofholding.webServe.serve_template(templatename="opds.html",
    ...
    def _root(self, **kwargs):
        ...
        feed = {'title': 'Bookbag of Holding OPDS', 'id': 'OPDSRoot', 'updated': now()}
        links = []
        entries = []
    ...
    def _Genres(self, **kwargs):
        ...
        myDB = database.DBConnection()
        feed = {'title': 'Bookbag of Holding OPDS - Genres', 'id': 'Genres', 'updated': now()}
        links = []
        entries = []
        links.append(getLink(href=self.opdsroot, ftype='application/atom+xml; profile=opds-catalog; kind=navigation',
                             rel='start', title='Home'))
    ...
    def _Genre(self, **kwargs):
        ...
        for book in page:
            ...
            if mime_type:
                ...
                if bookbagofholding.CONFIG['OPDS_METAINFO']:
        ...
        feed = {'title': 'Bookbag of Holding OPDS - Genre %s' % escape(kwargs['genre']),
    ...
    def _Authors(self, **kwargs):
        ...
        myDB = database.DBConnection()
        feed = {'title': 'Bookbag of Holding OPDS - Authors', 'id': 'Authors', 'updated': now()}
        links = []
        entries = []
        links.append(getLink(href=self.opdsroot, ftype='application/atom+xml; profile=opds-catalog; kind=navigation',
                             rel='start', title='Home'))
    ...
    def _Author(self, **kwargs):
        ...
        feed['title'] = 'Bookbag of Holding OPDS - %s' % authorname
    ...
    def _Books(self, sorder='Recent', **kwargs):
        ...
        myDB = database.DBConnection()
        feed = {'title': 'Bookbag of Holding OPDS - %s Books' % sorder, 'id': '%s Books' % sorder, 'updated': now()}
        links = []
        entries = []
        links.append(getLink(href=self.opdsroot, ftype='application/atom+xml; profile=opds-catalog; kind=navigation',
                             rel='start', title='Home'))
    ...
    def _Audio(self, sorder='Recent', **kwargs):
        ...
        myDB = database.DBConnection()
        feed = {'title': 'Bookbag of Holding OPDS - %s AudioBooks' % sorder, 'id': '%s AudioBooks' % sorder,
                'updated': now()}
        links = []
        entries = []
        links.append(getLink(href=self.opdsroot, ftype='application/atom+xml; profile=opds-catalog; kind=navigation',
