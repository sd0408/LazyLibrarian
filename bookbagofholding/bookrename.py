...
from bookbagofholding.common import safe_move
from bookbagofholding.formatter import plural, is_valid_booktype, check_int, replace_all, \
...
def audioProcess(bookid, rename=False, playlist=False):
    ...
    for item in ['$Part', '$Title']:
        if rename and item not in bookbagofholding.CONFIG['AUDIOBOOK_DEST_FILE']:
...
def bookRename(bookid):
    ...
    r = os.path.dirname(f)
    if not bookbagofholding.CONFIG['CALIBRE_RENAME']:
        try:
...
def nameVars(bookid, abridged=''):
    ...
    dest_path = replacevars(bookbagofholding.CONFIG['EBOOK_DEST_FOLDER'], mydict)
    dest_path = replace_all(dest_path, __dic__)
    mydict['FolderName'] = stripspaces(dest_path)

    bookfile = replacevars(bookbagofholding.CONFIG['EBOOK_DEST_FILE'], mydict)
    ...
    audiofile = replacevars(bookbagofholding.CONFIG['AUDIOBOOK_DEST_FILE'], mydict)
