...
from bookbagofholding.bookwork import getBookWork
from bookbagofholding.formatter import plural, makeUnicode, makeBytestr, safe_unicode, check_int, md5_utf8
from bookbagofholding.common import safe_copy, setperm
from bookbagofholding.cache import cache_img, fetchURL
...
def getBookCover(bookID=None, src=None):
    """ Return link to a local file containing a book cover image for a bookid, and which source used.
        Try 1. Local file cached from goodreads/googlebooks when book was imported
            ...
            8. Google images search (if bookbagofholding config allows)
    ...
    try:
        ...
        if not src or src == 'librarything':
            if bookbagofholding.CONFIG['LT_DEVKEY']:
                ...
                if item and item['BookISBN']:
                    img = 'https://www.librarything.com/devkey/%s/large/isbn/%s' % (
                           bookbagofholding.CONFIG['LT_DEVKEY'], item['BookISBN'])
                    if src:
                    ...
                    data = ''
                    coverfile = os.path.join(bookbagofholding.DATADIR, coverlink)
        ...
        if src == 'googleimage' or not src and bookbagofholding.CONFIG['IMP_GOOGLEIMAGE']:
...
def createMagCovers(refresh=False):
    if not bookbagofholding.CONFIG['IMP_MAGCOVER']:
...
def createMagCover(issuefile=None, refresh=False, pagenum=1):
    if not bookbagofholding.CONFIG['IMP_MAGCOVER'] or not pagenum:
        return 'unwanted'
    ...
    if data:
        ...
        generator = ""
        if len(bookbagofholding.CONFIG['IMP_CONVERT']):  # allow external convert to override libraries
            generator = "external program: %s" % bookbagofholding.CONFIG['IMP_CONVERT']
            if "gsconvert.py" in bookbagofholding.CONFIG['IMP_CONVERT']:
                ...
                logger.warn(msg)
            converter = bookbagofholding.CONFIG['IMP_CONVERT']
            postfix = ''
            # if not os.path.isfile(converter):  # full path given, or just program_name?
            #     converter = os.path.join(os.getcwd(), bookbagofholding.CONFIG['IMP_CONVERT'])
            ...
            try:
                ...
                if res:
                    logger.debug('%s reports: %s' % (bookbagofholding.CONFIG['IMP_CONVERT'], res))
            except Exception as e:
                # logger.debug(params)
        ...
        if os.path.isfile(coverfile):
            ...
            hashname = os.path.join(bookbagofholding.CACHEDIR, 'magazine', '%s.jpg' % myhash)
    ...
    try:
        coverfile = safe_copy(os.path.join(bookbagofholding.PROG_DIR, 'data/images/nocover.jpg'), coverfile)
        setperm(coverfile)
    except Exception as why:
