...
from bookbagofholding import logger, database, nzbget, sabnzbd, classes, utorrent, transmission, qbittorrent, \
...
from bookbagofholding.common import setperm, getUserAgent, proxyList, mymakedirs
from bookbagofholding.formatter import cleanName, unaccented_str, getList, makeUnicode
from bookbagofholding.postprocess import delete_task, check_contents
...
def NZBDownloadMethod(bookid=None, nzbtitle=None, nzburl=None, library='eBook'):
    ...
    if bookbagofholding.CONFIG['NZB_DOWNLOADER_SABNZBD'] and bookbagofholding.CONFIG['SAB_HOST']:
        Source = "SABNZBD"
        downloadID, res = sabnzbd.SABnzbd(nzbtitle, nzburl, False)  # returns nzb_ids or False

    if bookbagofholding.CONFIG['NZB_DOWNLOADER_NZBGET'] and bookbagofholding.CONFIG['NZBGET_HOST']:
        Source = "NZBGET"
    ...
    if bookbagofholding.CONFIG['NZB_DOWNLOADER_SYNOLOGY'] and bookbagofholding.CONFIG['USE_SYNOLOGY'] and \
            bookbagofholding.CONFIG['SYNOLOGY_HOST']:
        Source = "SYNOLOGY_NZB"
        downloadID, res = synology.addTorrent(nzburl)  # returns nzb_ids or False

    if bookbagofholding.CONFIG['NZB_DOWNLOADER_BLACKHOLE']:
        Source = "BLACKHOLE"
        ...
        if nzbfile:
            ...
            nzbpath = os.path.join(bookbagofholding.CONFIG['NZB_BLACKHOLEDIR'], nzbname)
            try:
...
def DirectDownloadMethod(bookid=None, dl_title=None, dl_url=None, library='eBook'):
    ...
    if not str(r.status_code).startswith('2'):
        ...
        if extn and extn in getList(bookbagofholding.CONFIG['EBOOK_TYPE']):
        ...
        destdir = os.path.join(bookbagofholding.DIRECTORY('Download'), basename)
        # destdir = os.path.join(bookbagofholding.DIRECTORY('Download'), '%s LL.(%s)' % (basename, bookid))
...
def TORDownloadMethod(bookid=None, tor_title=None, tor_url=None, library='eBook'):
    ...
    if bookbagofholding.CONFIG['TOR_DOWNLOADER_BLACKHOLE']:
        Source = "BLACKHOLE"
        ...
        if tor_url and tor_url.startswith('magnet'):
            if bookbagofholding.CONFIG['TOR_CONVERT_MAGNET']:
                hashid = calculate_torrent_hash(tor_url)
                if not hashid:
                    hashid = tor_name
                tor_name = 'meta-' + hashid + '.torrent'
                tor_path = os.path.join(bookbagofholding.CONFIG['TORRENT_DIR'], tor_name)
    ...
    if bookbagofholding.CONFIG['TOR_DOWNLOADER_UTORRENT'] and bookbagofholding.CONFIG['UTORRENT_HOST']:
    ...
    if bookbagofholding.CONFIG['TOR_DOWNLOADER_RTORRENT'] and bookbagofholding.CONFIG['RTORRENT_HOST']:
    ...
    if bookbagofholding.CONFIG['TOR_DOWNLOADER_QBITTORRENT'] and bookbagofholding.CONFIG['QBITTORRENT_HOST']:
        Source = "QBITTORRENT"
        if torrent:
    ...
    if bookbagofholding.CONFIG['TOR_DOWNLOADER_TRANSMISSION'] and bookbagofholding.CONFIG['TRANSMISSION_HOST']:
        Source = "TRANSMISSION"
        if torrent:
    ...
    if bookbagofholding.CONFIG['TOR_DOWNLOADER_SYNOLOGY'] and bookbagofholding.CONFIG['USE_SYNOLOGY'] and \
            bookbagofholding.CONFIG['SYNOLOGY_HOST']:
    ...
    if bookbagofholding.CONFIG['TOR_DOWNLOADER_DELUGE'] and bookbagofholding.CONFIG['DELUGE_HOST']:
            ...
            try:
                ...
                if downloadID:
                    if bookbagofholding.CONFIG['DELUGE_LABEL']:
                        _ = client.call('label.set_torrent', downloadID, bookbagofholding.CONFIG['DELUGE_LABEL'].lower())
    ...
    if downloadID:
        if tor_title:
            if downloadID.upper() in tor_title.upper():
                ...
                if library == 'Magazine':
                    reject_list = getList(bookbagofholding.CONFIG['REJECT_MAGS'], ',')
                elif library == 'eBook':
