...
from bookbagofholding.formatter import plural, check_int
from bookbagofholding.providers import IterateOverNewzNabSites, IterateOverTorrentSites, IterateOverRSSSites, \
    IterateOverDirectSites
from bookbagofholding.resultlist import findBestResult, downloadResult
...
def goodEnough(match):
    if match and int(match[0]) >= check_int(bookbagofholding.CONFIG['MATCH_RATIO'], 90):
        return True
    return False
    ...
    try:
        ...
        if 'rss' in modelist:
            ...
            if not nprov or (library == 'Audiobook' and 'A' not in dltypes) or \
                ...
                if check_int(bookbagofholding.NO_RSS_MSG, 0) + 1200 < timenow:
                    logger.warn('No rss providers are available. Check config and blocklist')
                    bookbagofholding.NO_RSS_MSG = timenow
                modelist.remove('rss')

        book_count = 0
        for book in searchlist:
            do_search = True
            if bookbagofholding.CONFIG['DELAYSEARCH'] and not force:
                ...
                for mode in modelist:
                    ...
                    if mode == 'nzb' and 'nzb' in modelist:
                        ...
                        if not nprov:
                            # don't nag. Show warning message no more than every 20 mins
                            timenow = int(time.time())
                            if check_int(bookbagofholding.NO_NZB_MSG, 0) + 1200 < timenow:
                                logger.warn('No nzb providers are available. Check config and blocklist')
                                bookbagofholding.NO_NZB_MSG = timenow
                            modelist.remove('nzb')
                        ...
                        if not nprov:
                            # don't nag. Show warning message no more than every 20 mins
                            timenow = int(time.time())
                            if check_int(bookbagofholding.NO_TOR_MSG, 0) + 1200 < timenow:
                                logger.warn('No tor providers are available. Check config and blocklist')
                                bookbagofholding.NO_TOR_MSG = timenow
                            modelist.remove('tor')
                        ...
                        if not nprov:
                            # don't nag. Show warning message no more than every 20 mins
                            timenow = int(time.time())
                            if check_int(bookbagofholding.NO_DIRECT_MSG, 0) + 1200 < timenow:
                                logger.warn('No direct providers are available. Check config and blocklist')
                                bookbagofholding.NO_DIRECT_MSG = timenow
            ...
            elif bookbagofholding.CONFIG['DELAYSEARCH'] and not force and do_search and len(modelist):
                res = myDB.match('SELECT * FROM failedsearch WHERE BookID=? AND Library=?',
                                 (book['bookid'], book['library']))
                if res:
                    interval = check_int(res['Interval'], 0)
