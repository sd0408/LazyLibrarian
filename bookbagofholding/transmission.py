...
def addTorrent(link, directory=None, metainfo=None):
    ...
    if not directory:
        directory = bookbagofholding.CONFIG['TRANSMISSION_DIR']
    if directory:
...
def removeTorrent(torrentid, remove_data=False):
    ...
    try:
        ...
        if finished:
            ...
            remove = True
        elif not bookbagofholding.CONFIG['SEED_WAIT']:
...
def torrentAction(method, arguments):
    ...
    username = bookbagofholding.CONFIG['TRANSMISSION_USER']
    password = bookbagofholding.CONFIG['TRANSMISSION_PASS']

    if host_url:
        if bookbagofholding.LOGLEVEL & bookbagofholding.log_dlcomms:
            logger.debug("Using existing host %s" % host_url)
    else:
        host = bookbagofholding.CONFIG['TRANSMISSION_HOST']
        port = check_int(bookbagofholding.CONFIG['TRANSMISSION_PORT'], 0)
        ...
        if not parts[2].endswith("/rpc"):
            if bookbagofholding.CONFIG['TRANSMISSION_BASE']:
                parts[2] += "/%s/rpc" % bookbagofholding.CONFIG['TRANSMISSION_BASE'].strip('/')
            else:
