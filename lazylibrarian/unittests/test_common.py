...
"""
Unit tests for lazylibrarian.common module.
...
class TestGetUserAgent:
    ...
    def test_getUserAgent_returns_custom_when_set(self):
        ...
        original = lazylibrarian.CONFIG.get('USER_AGENT', '')
        lazylibrarian.CONFIG['USER_AGENT'] = 'Custom User Agent/1.0'

        result = common.getUserAgent()
        assert result == 'Custom User Agent/1.0'

        lazylibrarian.CONFIG['USER_AGENT'] = original

    def test_getUserAgent_returns_default_when_not_set(self):
        """getUserAgent should return default user agent when not configured."""
        original = lazylibrarian.CONFIG.get('USER_AGENT', '')
        lazylibrarian.CONFIG['USER_AGENT'] = ''

        result = common.getUserAgent()
        assert 'LazyLibrarian' in result
...
class TestProxyList:
    ...
    def test_proxyList_returns_none_when_no_proxy(self):
        ...
        original = lazylibrarian.CONFIG.get('PROXY_HOST', '')
        lazylibrarian.CONFIG['PROXY_HOST'] = ''

        result = common.proxyList()
        assert result is None

        lazylibrarian.CONFIG['PROXY_HOST'] = original
    ...
    def test_proxyList_returns_dict_when_proxy_configured(self):
        ...
        original_host = lazylibrarian.CONFIG.get('PROXY_HOST', '')
        original_type = lazylibrarian.CONFIG.get('PROXY_TYPE', '')

        lazylibrarian.CONFIG['PROXY_HOST'] = 'http://proxy.example.com:8080'
        lazylibrarian.CONFIG['PROXY_TYPE'] = 'http, https'
        ...
        lazylibrarian.CONFIG['PROXY_HOST'] = original_host
        lazylibrarian.CONFIG['PROXY_TYPE'] = original_type
