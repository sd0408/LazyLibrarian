...
"""
Pytest configuration and shared fixtures for LazyLibrarian tests.
"""
...
# Ensure lazylibrarian package is importable
...
def setup_lazylibrarian_globals():
    """Initialize LazyLibrarian global variables needed for tests."""
    ...
    lazylibrarian.DATADIR = tempfile.mkdtemp(prefix='ll_test_')
    lazylibrarian.SYS_ENCODING = 'utf-8'
    lazylibrarian.LOGLEVEL = 0  # Disable debug logging during tests
    lazylibrarian.CURRENT_BRANCH = 'master'

    # Initialize CONFIG dict if not present
    if not hasattr(lazylibrarian, 'CONFIG') or lazylibrarian.CONFIG is None:
    ...
    lazylibrarian.CONFIG.setdefault('PROXY_HOST', '')
    lazylibrarian.CONFIG.setdefault('PROXY_TYPE', '')
    lazylibrarian.CONFIG.setdefault('LOGLIMIT', 500)
    lazylibrarian.CONFIG.setdefault('LOGDIR', os.path.join(lazylibrarian.DATADIR, 'Logs'))
    lazylibrarian.CONFIG.setdefault('DIR_PERM', '0o755')
    lazylibrarian.CONFIG.setdefault('FILE_PERM', '0o644')
    lazylibrarian.CONFIG.setdefault('GIT_PROGRAM', '')
    lazylibrarian.CONFIG.setdefault('GIT_HOST', 'github.com')
    lazylibrarian.CONFIG.setdefault('GIT_USER', 'lazylibrarian')
    lazylibrarian.CONFIG.setdefault('GIT_REPO', 'LazyLibrarian')
    lazylibrarian.CONFIG.setdefault('GIT_BRANCH', 'master')
    lazylibrarian.CONFIG.setdefault('GIT_UPDATED', 0)
    lazylibrarian.CONFIG.setdefault('LOGLEVEL', 0)
    lazylibrarian.CONFIG.setdefault('HTTP_EXT_TIMEOUT', 90)
    lazylibrarian.CONFIG.setdefault('CACHE_AGE', 30)

    # Create log directory
    log_dir = lazylibrarian.CONFIG['LOGDIR']
    ...
    if not hasattr(lazylibrarian, 'logger') or lazylibrarian.logger is None:
        lazylibrarian.logger = MockLogger()
    ...
    if os.path.exists(lazylibrarian.DATADIR):
        shutil.rmtree(lazylibrarian.DATADIR, ignore_errors=True)
...
def temp_db():
    ...
    original_dbfile = getattr(lazylibrarian, 'DBFILE', None)
    lazylibrarian.DBFILE = db_path
    ...
    # Cleanup
    conn.close()
    lazylibrarian.DBFILE = original_dbfile
...
def mock_config():
    ...
    affecting the global lazylibrarian.CONFIG.
    """
    return {
        'HTTP_PORT': 5299,
        'HTTP_HOST': '0.0.0.0',
...
def mock_logger(monkeypatch):
    """
    Replace the lazylibrarian logger with a mock for testing.

    Returns the mock logger instance so tests can inspect logged messages.
    """
    mock = MockLogger()
    monkeypatch.setattr(lazylibrarian, 'logger', mock)
    # Also patch the logger module reference if it exists
    if hasattr(lazylibrarian, 'logger'):
        monkeypatch.setattr('lazylibrarian.logger', mock)
    return mock
...
def api_config():
    ...
    # Set up API config
    lazylibrarian.CONFIG['API_ENABLED'] = True
    lazylibrarian.CONFIG['API_KEY'] = 'a' * 32  # Valid 32-char key
    ...
    if not hasattr(lazylibrarian, 'LOGLIST'):
        lazylibrarian.LOGLIST = []

    yield lazylibrarian.CONFIG
...
def authenticated_user(temp_db):
    ...
    db.action(
        ...
        [user_id, username, password_hash, 65535]  # 65535 = perm_admin (all permissions)
    )
