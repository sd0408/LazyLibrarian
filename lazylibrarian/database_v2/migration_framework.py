...
"""
Database migration framework for LazyLibrarian.
...
class Migration(ABC):
    ...
    def log(self, message: str) -> None:
        ...
        lazylibrarian.UPDATE_MSG = message
...
class MigrationRunner:
    ...
    def __init__(self, db: Any, log_dir: Optional[str] = None):
        ...
        """
        self.db = db
        self.log_dir = log_dir or lazylibrarian.CONFIG.get('LOGDIR', '')
        self._log_file = None
    ...
    def run(self, target_version: Optional[int] = None) -> bool:
        ...
        try:
            lazylibrarian.UPDATE_MSG = 'Updating database from version %d to %d' % (current, target)
            ...
            lazylibrarian.UPDATE_MSG = 'Database updated to version %d' % target
            logger.info(lazylibrarian.UPDATE_MSG)
            if self._log_file:
                self._log_file.write("%s: %s\n" % (time.ctime(), lazylibrarian.UPDATE_MSG))
    ...
    def _run_migration(self, migration_class: Type[Migration]) -> None:
        ...
        lazylibrarian.UPDATE_MSG = 'Running migration v%d: %s' % (
    ...
    def rollback(self, target_version: int) -> bool:
        ...
        try:
            lazylibrarian.UPDATE_MSG = 'Rolling back database from version %d to %d' % (current, target_version)
            logger.info(lazylibrarian.UPDATE_MSG)
            if self._log_file:
                self._log_file.write("%s: %s\n" % (time.ctime(), lazylibrarian.UPDATE_MSG))
            ...
            for version in rollback_versions:
                ...
                if migration_class:
                    ...
                    lazylibrarian.UPDATE_MSG = 'Rolling back migration v%d' % version
                    logger.info(lazylibrarian.UPDATE_MSG)
                    migration.down()
            ...
            lazylibrarian.UPDATE_MSG = 'Database rolled back to version %d' % target_version
            logger.info(lazylibrarian.UPDATE_MSG)
            if self._log_file:
                self._log_file.write("%s: %s\n" % (time.ctime(), lazylibrarian.UPDATE_MSG))
